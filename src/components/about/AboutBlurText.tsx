...
export function AboutBlurText({
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      if (onLoad) {
        ...
        const onWordDone = () => {
          finished += 1
          if (finished === words.length && root) {
            root.classList.remove('about-blur-on-load')
  ...
  return (
    <Tag
      ref={ref as never}
      className={cn(className, onLoad && 'about-blur-on-load')}
    >
      {parts.map((word, i) => (
        <span
          key={`${word}-${i}`}
          className="ab-word inline-block mr-[0.26em] align-top"
